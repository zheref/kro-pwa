...
const reduce = (
  state: EarnState,
  action: Parameters<typeof earnSlice.reducer>[1],
...
describe('userDidTapAddReward', () => {
  ...
  it('falls back to 100 when the preference has not loaded yet', () => {
    expect(
      reduce(initialEarnState, userDidTapAddReward()).addRewardDraft
        .pointsRequired,
    ).toBe(100)
  })
})
...
describe('draft field edits', () => {
  it('userDidChangeDraftTitle sets the title', () => {
    expect(
      reduce(loaded, userDidChangeDraftTitle({ title: 'Boba Tea' }))
        .addRewardDraft.title,
    ).toBe('Boba Tea')
  })

  it('userDidChangeDraftGlyph truncates to two code points', () => {
    expect(
      reduce(loaded, userDidChangeDraftGlyph({ glyph: 'xyz' })).addRewardDraft
        .glyph,
    ).toBe('xy')
  })
  ...
  it('userDidChangeDraftPoints clamps to zero', () => {
    expect(
      reduce(loaded, userDidChangeDraftPoints({ pointsRequired: -10 }))
        .addRewardDraft.pointsRequired,
    ).toBe(0)
  })

  it('userDidChangeDraftNotes maps empty to null', () => {
    expect(
      reduce(loaded, userDidChangeDraftNotes({ notes: '' })).addRewardDraft
        .notes,
    ).toBeNull()
  })
})
...
describe('userDidTapClaim / userDidCancelClaim', () => {
  ...
  it('cancel clears the confirm sheet', () => {
    const opened = reduce(
      loaded,
      userDidTapClaim({ rewardId: rewardMocks.bobaTea.id }),
    )
  ...
  it('re-tapping a different reward replaces the pointer', () => {
    ...
    const second = reduce(
      first,
      userDidTapClaim({ rewardId: rewardMocks.plain.id }),
    )
  ...
  it('applies the loaded preferences on success', () => {
    const next = reduce(
      ...
      loadEarnPreferencesThunk.fulfilled(
        {
          ok: true,
          value: { defaultRewardThreshold: 300, pointsFormula: 'legacy' },
        },
        'req',
        undefined,
      ),
...
describe('loadEarnCatalogThunk lifecycle', () => {
  it('shows the spinner on pending, clearing any prior exception', () => {
    const failed = {
      ...loaded,
      load: earnStateMocks.failedRefreshKeepingCatalog.load,
    }
    expect(
      reduce(failed, loadEarnCatalogThunk.pending('req', undefined)).load,
    ).toEqual({
      kind: 'loading',
    })
  })
  ...
  it('installs the whole snapshot on success', () => {
    const snapshot = {
      rewards: [rewardMocks.movieNight],
      claimedRewardIds: [],
      performances: [],
    }
    const next = reduce(
  ...
  it('reports the typed failure, keeping the retained catalog (canon: never blank the surface)', () => {
    const next = reduce(
      ...
      loadEarnCatalogThunk.fulfilled(
        {
          ok: false,
          error: { kind: 'catalogLoadFailed', message: 'x', recoverable: true },
        },
        'req',
        undefined,
      ),
  ...
  it('stays silent on an aborted read', () => {
    const next = reduce(
      loaded,
      loadEarnCatalogThunk.rejected(abortError(), 'req', undefined),
    )
    expect(next.load).toEqual(loaded.load)
  })
})
...
describe('addRewardThunk lifecycle', () => {
  ...
  it('prepends the new reward on success', () => {
    const next = reduce(
      ...
      addRewardThunk.fulfilled(
        { ok: true, value: { reward: rewardMocks.movieNight } },
        'req',
        arg,
      ),
    )
  ...
  it('reports the typed failure and leaves the sheet open for a retry', () => {
    ...
    const next = reduce(
      ...
      addRewardThunk.fulfilled(
        {
          ok: false,
          error: { kind: 'addRewardFailed', message: 'x', recoverable: true },
        },
        'req',
        arg,
      ),
  ...
  it('degrades an unexpected rejection to the generic exception', () => {
    const next = reduce(
      loaded,
      addRewardThunk.rejected(new Error('kaboom'), 'req', arg),
    )
    expect(next.load.kind).toBe('failed')
  })
})
...
describe('addSuggestionThunk lifecycle', () => {
  ...
  it('prepends the inserted suggestion on success', () => {
    const next = reduce(
      ...
      addSuggestionThunk.fulfilled(
        {
          ok: true,
          value: { reward: { ...rewardMocks.bobaTea, id: 'new-2' } },
        },
        'req',
        arg,
      ),
  ...
  it('degrades an unexpected rejection to the generic exception', () => {
    const next = reduce(
      loaded,
      addSuggestionThunk.rejected(new Error('kaboom'), 'req', arg),
    )
    expect(next.load.kind).toBe('failed')
  })
})
...
describe('deleteRewardThunk lifecycle', () => {
  ...
  it('removes the reward on success', () => {
    const next = reduce(
      ...
      deleteRewardThunk.fulfilled(
        { ok: true, value: { id: rewardMocks.bobaTea.id } },
        'req',
        arg,
      ),
    )
    expect(next.rewards.some((r) => r.id === rewardMocks.bobaTea.id)).toBe(
      false,
    )
  })
  ...
  it('degrades an unexpected rejection to the generic exception', () => {
    const next = reduce(
      loaded,
      deleteRewardThunk.rejected(new Error('kaboom'), 'req', arg),
    )
    expect(next.load.kind).toBe('failed')
  })
})
...
describe('claimRewardThunk lifecycle — the atomic one', () => {
  ...
  it('leaves the claimed set and the confirm sheet untouched on a typed failure — atomicity', () => {
    ...
    const next = reduce(
      ...
      claimRewardThunk.fulfilled(
        {
          ok: false,
          error: { kind: 'claimRewardFailed', message: 'x', recoverable: true },
        },
        'req',
        arg,
      ),
  ...
  it('stays silent on an aborted claim', () => {
    const next = reduce(
      loaded,
      claimRewardThunk.rejected(abortError(), 'req', arg),
    )
    expect(next.load).toEqual(loaded.load)
  })
})
